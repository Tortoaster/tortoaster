<script lang="ts">
    import {PUBLIC_BUCKET_URL} from '$env/static/public';
    import type {PageData} from './$types';
    import Throbber from "$lib/Throbber.svelte";
    import Overlay from "$lib/Overlay.svelte";
    import {format} from "date-fns";

    interface Props {
        data: PageData;
    }

    const {data}: Props = $props();
</script>

<svelte:head>
    {#await data.project}
        <title>Tortoaster</title>
    {:then project}
        <title>{project.name} - Tortoaster</title>
    {:catch _}
        <title>Tortoaster</title>
    {/await}
</svelte:head>

<Overlay>
    {#await data.project}
        <Throbber class="text-white-brightest"/>
    {:then project}
        {@const [day, date, year] = format(project.datePosted, 'PPPP').split(', ')}
        <div class="flex justify-between items-baseline">
            <h1 class="text-4xl text-white-bright font-bold">{project.name}</h1>
            <div class="flex">
                <span class="text-white italic hidden lg:block">{day},&nbsp;</span>
                <span class="text-white italic">{date}</span>
                <span class:hidden={+year !== new Date().getFullYear()} class="text-white italic">,&nbsp;{year}</span>
            </div>
        </div>
    {/await}
</Overlay>

<main class="flex flex-col gap-single p-single">
    {#await data.project}
        <Throbber class="text-black-darker"/>
    {:then project}
        <div class="card mx-auto">
            <img alt="Thumbnail" src="{PUBLIC_BUCKET_URL}/thumbnails/{project.thumbnailId}">
        </div>

        {#if project.projectUrl}
            <a class="btn text-lg mx-auto" href={project.projectUrl}>Visit project page &#x2197;</a>
        {/if}
    {/await}

    <div class="md lg:max-w-gratio lg:mx-auto">
        {#await data.content}
            <Throbber class="text-black-darker"/>
        {:then content}
            {@html content}
        {/await}
    </div>
</main>
